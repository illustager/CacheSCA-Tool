...
int load_data(const config_t *config, aes_sample_t samples[]);
void test_mode_0(const aes_sample_t *samples, int nsamples);
void test_mode_1(const aes_sample_t *samples, int nsamples);
void test_mode_2(const aes_sample_t *samples, int nsamples);
bool block_equal(const aes_block_t a, const aes_block_t b);
...
int main(int argc, char *argv[]) {
	...
	aes_sample_t samples[config.nsamples];
...
void test_mode_0(const aes_sample_t *samples, int nsamples) {
	...
	puts("Testing AES-128 Encryption...");

	(void)timing();
	for (int i = 0; i < nsamples; ++i) {
		SetEncryptKey128(&ctx, samples[i].key);
		CRYPT_AES_Encrypt(&ctx, samples[i].plaintext, out, sizeof(aes_block_t));
	...
	puts("Testing AES-128 Decryption...");

	(void)timing();
	for (int i = 0; i < nsamples; ++i) {
		SetDecryptKey128(&ctx, samples[i].key);
		CRYPT_AES_Decrypt(&ctx, samples[i].ciphertext, out, sizeof(aes_block_t));
...
void test_mode_1(const aes_sample_t *samples, int nsamples) {
	...
	puts("Testing AES-128 Key Expansion...");
	...
	clock_t enc_time = timing();
	printf("Key expansion time for %d encryption keys: %ld clock ticks\n", nsamples, enc_time);

	for (int i = 0; i < nsamples; ++i) {
		SetDecryptKey128(&ctx, samples[i].key);
	}
	clock_t dec_time = timing();
	printf("Key expansion time for %d decryption keys: %ld clock ticks\n", nsamples, dec_time);
...
void test_mode_2(const aes_sample_t *samples, int nsamples) {
	...
	puts("Testing AES-128 Repeated Encryption...");

	(void)timing();
	SetEncryptKey128(&ctx, samples[0].key);
	for (int i = 0; i < nsamples; ++i) {
		CRYPT_AES_Encrypt(&ctx, samples[i].plaintext, out, sizeof(aes_block_t));
	}
	clock_t enc_time = timing();
	...
	puts("Testing AES-128 Repeated Decryption...");

	(void)timing();
	SetDecryptKey128(&ctx, samples[0].key);
	for (int i = 0; i < nsamples; ++i) {
		CRYPT_AES_Decrypt(&ctx, samples[i].ciphertext, out, sizeof(aes_block_t));
	}
	clock_t dec_time = timing();
...
bool block_equal(const aes_block_t a, const aes_block_t b) {
	return memcmp(a, b, sizeof(aes_block_t)) == 0;
}
